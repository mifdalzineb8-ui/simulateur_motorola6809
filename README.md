...
- `BRN` (branchement jamais)
- Conditions : `BHI`, `BLS`, `BCC`, `BCS`, `BNE`, `BEQ`, `BVC`, `BVS`, `BPL`, `BMI`, `BGE`, `BLT`, `BGT`, `BLE`
...
- **Relatif** : Pour les instructions de branchement
...
2. Implémenter la logique dans `clsExecuter.java` et `clsPasàpas.java`
